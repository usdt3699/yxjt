<template>
  <div class="min-h-screen bg-[#F9B48E] relative  pb-[10px]">
            ...
            <div class="pt-[60px] pb-2 px-[15px]">
              ...
              >
                ...
                <div class="flex justify-between px-[10px]">
                  <button class="view-button" @click="jumpToPage('houseDetails', item)">查看户型</button>
                  <button
                      class="free-button"
  ...
  methods: {
    ...
    async getAssetList() {
      try {
        ...
        const res = await axios.get(api.myHouseListSec);
        if (res.code === 200 && res.data?.houseList) {
          this.assetList = res.data.houseList;
        }
      } catch (error) {
        Dialog.alert({
          title: "提示",
    ...
    applyHouse(item) {
      this.$router.push({
        name: 'applyHouse', query: {
          id: item.id,
          receiverName: item.house_detail?.real_name || '',
