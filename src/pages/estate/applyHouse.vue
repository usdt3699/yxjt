    ...
    <div class="w-full relative">
      ...
      <div class="absolute top-0 left-0 right-0 w-full">
        ...
        <div class="content-container mx-4 rounded-lg p-4">
          <!-- 标题 -->
          <div class="apply-title">申领房产填写资料</div>
          ...
          <div class="form-container">
            ...
            <!-- 入住人数 -->
            <div class="form-item">
              <div class="label">入住人数<span class="required">*</span></div>
              <input type="number" class="input-field" placeholder="请输入入住人数" v-model="formData.occupancy">
            </div>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      if (!this.formData.id) {
        Dialog.alert({message: '参数错误，请重新选择房产'});
        return false;
      }
      ...
      if (!this.formData.occupancy) {
        Dialog.alert({message: '请输入入住人数'});
        return false;
      }
    ...
    async submitForm(isSelf) {
      ...
      try {
        ...
        formData.append('occupancy', this.formData.occupancy);
        formData.append('is_self', isSelf);

        // 发送请求
        const res=await axios.post(api.fetchHouse, formData);
        Dialog.alert({
          message: res.msg||'申领成功',
          confirmButtonText: '确定',
          confirmButtonColor: '#EB6400'
        }).then(() => {
          this.$router.back()
          // this.$router.replace({
          //   name: 'depositReceipt', query: {
          //     id: res.data.id,
