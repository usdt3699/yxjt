  ...
  methods: {
    ...
    drawCertificate() {
      ...
      certificateImg.onload = () => {
        ...
        ctx.fillText(cardInfo.get_name || '', width / 2 - 96, height / 2 - 90);

        ctx.font = '7px "SimSun"';
        ctx.fillStyle = '#333333';
        ctx.fillText(`${cardInfo.get_province|| ''} ${cardInfo.get_city|| ''} ${cardInfo.get_district|| ''}`, width / 2 -10, height / 2 - 68);

        ctx.font = '7px "SimSun"';
        ctx.fillStyle = '#333333';
        ctx.fillText(`${cardInfo.get_province|| ''} ${cardInfo.get_city|| ''} ${cardInfo.get_district|| ''}`, width / 2 +46, height / 2 - 46);


        // 绘制底部日期（使用created_at）
        if (cardInfo.zhihuan_time) {
          try {
            // 将日期拆分成年月日

            const date = moment(cardInfo.zhihuan_time);
            if (date.isValid()) {
              ...
              ctx.font = '7px "SimSun"';
              ctx.fillStyle = '#333333';
              ctx.fillText(createdYear, width / 2 + 60, height - 130);
              ctx.fillText(createdMonth, width / 2 + 92, height - 130);
              ctx.fillText(createdDay, width / 2 + 117, height - 130);
            }
          } catch (e) {
      ...
      certificateImg.src = require('../../assets/img/invite/model_exchange.jpg');
    }
  }
};
</script>
