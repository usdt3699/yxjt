  ...
  methods: {
    ...
    drawCertificate() {
      ...
      certificateImg.onload = () => {
        ...
        // 获取卡详情数据
        const carDetail= this.cardDetail.car_detail;
        // 设置字体样式
        ctx.textAlign = 'left';
        ...
        ctx.fillText(carDetail.get_name || '', width / 2 - 96, height / 2 - 90);
        ...
        if (carDetail.created_at) {
          try {
            ...
            const date = moment(carDetail.created_at);
            if (date.isValid()) {
              ...
              ctx.font = '7px "SimSun"';
              ctx.fillStyle = '#333333';
              ctx.fillText(createdYear, width / 2 + 60, height - 140);
              ctx.fillText(createdMonth, width / 2 + 92, height - 140);
              ctx.fillText(createdDay, width / 2 + 117, height - 140);
            }
          } catch (e) {
      ...
      try {
        if(this.cardDetail.id==1){
          certificateImg.src = require('../../assets/img/invite/model_1.jpg');
        }
        else if(this.cardDetail.id==2){
          certificateImg.src = require('../../assets/img/invite/model_2.jpg');
        }
        else if(this.cardDetail.id==3){
          certificateImg.src = require('../../assets/img/invite/model_3.jpg');
        }
